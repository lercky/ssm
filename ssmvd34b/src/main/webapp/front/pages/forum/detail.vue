...
<script>
	export default {
		...
		methods: {
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussforum/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('forum', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('forum', this.detail);
				this.$utils.msg('审核成功');
