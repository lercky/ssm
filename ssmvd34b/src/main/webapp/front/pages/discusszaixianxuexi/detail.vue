...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusszaixianxuexi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscusszaixianxuexi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusszaixianxuexi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusszaixianxuexi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusszaixianxuexi', this.detail);
				this.$utils.msg('审核成功');
