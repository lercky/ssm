...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshipinxuexi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussshipinxuexi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussshipinxuexi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussshipinxuexi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshipinxuexi', this.detail);
				this.$utils.msg('审核成功');
