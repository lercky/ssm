	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					用户名：{{detail.username}}
                				                				...
                				                				<view style="margin-top: 10upx;">
					试卷id（外键）：{{detail.paperid}}
				</view>
                				                				<view style="margin-top: 10upx;">
					试卷名称：{{detail.papername}}
				</view>
                				                				<view style="margin-top: 10upx;">
					试题id（外键）：{{detail.questionid}}
				</view>
                				                				<view style="margin-top: 10upx;">
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('examrecord', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussexamrecord/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('examrecord', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('examrecord', this.detail);
				this.$utils.msg('审核成功');
