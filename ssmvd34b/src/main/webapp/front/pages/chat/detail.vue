		...
		<view class="container">
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				<view style="text-align: left;display: flex;">
										<button @tap="onChatTap" class="action">
						<view>
							<view class="cuIcon-service text-yellow"></view>
							客服
						</view>
					</button>
									</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusschat/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('chat', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('chat', this.detail);
				this.$utils.msg('审核成功');
