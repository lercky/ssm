		...
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					<image mode="aspectFill" :src="swiper"></image>
                                                ...
                                                <view class="name">
					教学标题：{{detail.jiaoxuebiaoti}}
				</view>
				                                                                                                                                                                                                                                                                                				
                                								                								                								                								                				<view style="margin-top: 10upx;">
					发布时间：{{detail.fabushijian}}
				</view>
                				                								                								                                				                								                
				                <view style="margin-top: 20upx;display: flex;align-items: center;justify-content: space-between;">
				...
				<view style="margin-top: 20upx;display: flex;align-items: center;justify-content: space-between;">
					...
					<image @click="cai" class="icon" src="../../static/gen/cai.png" mode=""></image>
								...
								<view style="margin-top: 20upx;display: flex;align-items: center;justify-content: space-between;">
					收藏
					<image @click="shoucang" class="icon" src="../../static/gen/shoucang.png" mode=""></image>
				</view>
				
			</view>

                        			            			                        <view style="text-align: center;padding: 10upx;background: #FFFFFF;">
				<view class="uni-padding-wrap uni-common-mt">
					<view>
						<video id="myVideo" :src="detail.jiaoxueshipin" controls></video>
					</view>
				</view>
			</view>
				...
				<view class="comoment-header">
					...
					<view @click="onCommentTap" class="btn-comment-content" style="display: flex;align-items: center;">
						<image class="icon" src="../../static/gen/tianjia.png" mode=""></image> 添加评论
					</view>
				</view>

				<view class="cu-list comment" style="margin-top: 20upx;">
					<view v-for="(item,index) in commentList" v-bind:key="index" class="cu-item comment-item">
						<view class="content">
							<view class="text-grey">匿名用户{{item.userid}}</view>
							<view class="text-gray text-content text-df content">
								{{item.content}}
							</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('shipinxuexi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussshipinxuexi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussshipinxuexi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('shipinxuexi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('shipinxuexi', this.detail);
				this.$utils.msg('审核成功');
