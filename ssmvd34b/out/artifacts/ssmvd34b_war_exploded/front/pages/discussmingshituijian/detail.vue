...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussmingshituijian', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussmingshituijian', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussmingshituijian/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussmingshituijian', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussmingshituijian', this.detail);
				this.$utils.msg('审核成功');
