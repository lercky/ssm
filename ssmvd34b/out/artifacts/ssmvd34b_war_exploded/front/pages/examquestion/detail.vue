<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					所属试卷id（外键）：{{detail.paperid}}
				</view>
                				                				<view style="margin-top: 10upx;">
					试卷名称：{{detail.papername}}
				</view>
                				                				<view style="margin-top: 10upx;">
					试题名称：{{detail.questionname}}
				</view>
                				                								                				<view style="margin-top: 10upx;">
					分值：{{detail.score}}
				</view>
					...
					正确答案：{{detail.answer}}
                				                								                				...
                				                								                				<view style="margin-top: 10upx;">
					试题类型，0：单选题 1：多选题 2：判断题 3：填空题（暂不考虑多项填空）：{{detail.type}}
				</view>
                				                				<view style="margin-top: 10upx;">
					试题排序，值越大排越前面：{{detail.sequence}}
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('examquestion', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussexamquestion/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('examquestion', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('examquestion', this.detail);
				this.$utils.msg('审核成功');
