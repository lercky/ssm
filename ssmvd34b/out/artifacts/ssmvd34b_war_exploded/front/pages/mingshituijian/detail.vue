		...
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					...
					课程类型：{{detail.kechengleixing}}
                				                				...
                				                				<view style="margin-top: 10upx;">
					适合专业：{{detail.shihezhuanye}}
				</view>
                				                				<view style="margin-top: 10upx;">
					课程价格：{{detail.kechengjiage}}
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('mingshituijian', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussmingshituijian', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussmingshituijian/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('mingshituijian', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('mingshituijian', this.detail);
				this.$utils.msg('审核成功');
